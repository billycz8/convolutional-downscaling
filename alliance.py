...
#%% 
train_extra = np.tile(np.expand_dims(np.tile(lat_long_top_reshape,reps=train_inputs.shape[0]),axis=-1),3).transpose(0,3,4,1,2)
val_extra = np.tile(np.expand_dims(np.tile(lat_long_top_reshape,reps=val_inputs.shape[0]),axis=-1),3).transpose(0,3,4,1,2)
# %%
...
checkpoint_callback = ModelCheckpoint(
    filepath='best_model_all.keras',  # Filepath where the model will be saved
...
#%% 
test_extra = np.tile(np.expand_dims(np.tile(lat_long_top_reshape,reps=test_inputs.shape[0]),axis=-1),3).transpose(0,3,4,1,2)

# Load the saved model
best_model = load_model('best_model_all.keras',custom_objects={'grad_loss': grad_loss, "transpose_fn": transpose_fn})
